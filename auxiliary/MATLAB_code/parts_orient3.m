...
function [action_pcl_tf, grasp_pcl_tf, max_grasp_x, min_action_x, att_point_action, att_point_grasp] = parts_orient3(action_match, grasp_match, angle_relative, pos_relative, part_location, names, num_att, att_type)
    ...
    tform_grasp = makehgtform('translate', [diff,0,0]);
    ...
    min_action_pt = min(action_object.Location);
    min_action_x = [min_action_pt(1), action_mean_loc(2), action_mean_loc(3)];
    ...
    max_grasp_pt = max(grasp_object.Location);
    max_grasp_x = [max_grasp_pt(1), grasp_mean_loc(2), grasp_mean_loc(3)];
    ...
    if strcmp(att_type, 'grasp')
        [att_point_action, att_point_grasp] = att_retrieval(action_match, grasp_match, action_pcl_tf, grasp_pcl_tf, names, num_att);
    end
