...
def astar_search(task, heuristic, make_open_entry=ordered_node_astar,
    ...
    while open:
        ...
        if (state_cost[pop_state] == pop_node.g) and (state_cost[pop_state] != float('inf')):
            expansions += 1
            ...
            for op, succ_state in task.get_successor_states(pop_state):
                ...
                if 'join-' in op.name:
                    ...
                    # Compute visual score if flag enabled
                        ...
                        if attCompute(action_part, handle_part):
                            visual_score = scoreCompute(action_part, handle_part, tool_type)
                        else:
                            visual_score = -float('inf')

                        # Check flag for sensor_trust parameter
                        if settings_flag['trust'] and visual_score == -float('inf'):
                            # Use shape score only if visual_score is infinity
                            visual_score = scoreCompute(action_part, handle_part, tool_type, True)
                ...
                h = heuristic(succ_node) - visual_score if settings_flag['ff_flag'] else 2.0 - visual_score
