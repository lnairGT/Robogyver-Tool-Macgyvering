...
def breadth_first_search(planning_task):
    ...
    while queue:
        ...
        if planning_task.goal_reached(node.state):
            ...
            return node.extract_solution(), iteration
        for operator, successor_state in planning_task.get_successor_states(
                                                                   node.state):
            # if parsed operator objects are in object combination, skip the iteration
            if 'join-' in operator.name:
                action_part, handle_part, _ = actionParser(operator)
