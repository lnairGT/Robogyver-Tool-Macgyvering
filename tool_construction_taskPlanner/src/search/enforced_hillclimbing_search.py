...
def enforced_hillclimbing_search(planning_task, heuristic,
    ...
    while queue:
        ...
        if planning_task.goal_reached(node.state):
            ...
            return node.extract_solution(), len(visited)
        ...
        for operator, successor_state in planning_task.get_successor_states(
            ...
            if successor_state not in closed:
                ...
                if 'join-' in operator.name:
                    # Keep track of constructions that are already attempted
                    action_part, handle_part, tool_type = actionParser(operator)
                ...
                heuristic_value = heuristic(successor_node) - visual_score if settings_flag['ff_flag'] else 2.0 - visual_score
